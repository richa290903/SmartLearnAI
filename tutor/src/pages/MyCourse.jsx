...
function Course() {
  ...
  const fetchdata = async () => {
    try {
      const response = await Api.get("/get_course_by_instructor_id");
      setCourseData(response.data);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {coursedata.length > 0 ? (
            coursedata.map((data) => (
              <div
                key={data.course_id}
                className="block shadow-md rounded-xl overflow-hidden hover:shadow-xl transition duration-300 bg-white"
              >
                {/* Click whole card → course detail */}
                <div
                  onClick={() => navigate(`/coursedisplay/${data.course_id}`)}
                ...
                <div className="p-4 pt-0">
                  <button
                    onClick={(e) => {
                      ...
                      navigate(`/module/upload/${data.course_id}`);
                    }}
